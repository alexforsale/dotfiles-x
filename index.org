...
For most distribution that uses /systemd/, this should be run with ~systemctl --user start dunst~, I'm just too lazy to add a condition for it.
#+begin_src sh -n
...
***** 04-bluetooth.sh
:PROPERTIES:
:header-args: :tangle .config/xprofile.d/04-bluetooth.sh :shebang #!/bin/sh
:END:
